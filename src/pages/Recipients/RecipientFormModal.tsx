...
import type { RecipientRequest, RecipientResponse } from '../../types/api.types';

interface RecipientFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: RecipientRequest) => Promise<void>;
...
export default function RecipientFormModal({
  ...
  const [formData, setFormData] = useState<RecipientRequest>({
    firstName: '',
    lastName: '',
    email: '',
    phoneNumber: '',
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={recipient ? 'Modifier le destinataire' : 'Nouveau destinataire'}
      size="lg"
      showFooter
      footerContent={
